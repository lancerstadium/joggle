...
model then derives its exact `quant` dependency from its Function calls. FLOAT
...
QDQ calls belong to the independent `quant` Module; all other calls remain in
...
Module is returned. Intermediate metadata, inferred shapes, element Types,
...
joggle run driver.joggle read squeezenet1.0-13-qdq.onnx \
  --with /path/to/tensor/module.joggle \
  --with /path/to/quant/module.joggle \
  --with /path/to/onnx/module.joggle \
  --load-native onnx=/path/to/onnx/native \
