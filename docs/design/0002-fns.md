...
1. parameter type expressions are resolved in the surrounding mod scope;
2. a fresh anonymous `Fn` is created in the same compiler closure;
...
6. the owning fn receives an inline callable `Val` with the derived
