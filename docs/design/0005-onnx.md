# Design 0005: ONNX import

Status: accepted

## Purpose

The first ONNX path must prove that an external model can become an ordinary
Joggle `Mod` without adding a frontend class, graph container, pass kind, or
ONNX operation hierarchy to compiler core. The contract is deliberately an
inference importer rather than a claim of general ONNX coverage.

```joggle
joggle 1;

mod onnx@2.0.0 {
  import arith@1 as a;
  import tensor@2 as t;
  fn read(input: bytes, name: string = "model") -> mod;
}
```

`read` is a normal explicitly staged compiler fn. `name` supplies a
valid deterministic name for the returned Mod; a file path is not hidden in
the byte value or native binding.

## Reference artifact

The first vertical slice is the ONNX Model Zoo SqueezeNet 1.1 artifact:

| Field | Audited value |
| --- | --- |
| Repository | `onnxmodelzoo/squeezenet1.1-7` |
| File | `squeezenet1.1-7.onnx` |
| SHA-256 | `1eeff551a67ae8d565ca33b572fc4b66e3ef357b0eb2863bb9ff47a918cc4088` |
| Size | 4,956,208 bytes |
| ONNX IR / opset | IR 3 / `ai.onnx` 7 |
| Graph | `main`, 66 nodes, 53 initializers, no `value_info` |
| Input / output | `f32[1,3,224,224]` / `f32[1,1000]` |

The model was decoded with the official ONNX 1.19.0 `onnx.proto`. Its node
multiset is Conv 26, Relu 26, Concat 8, MaxPool 3, AveragePool 1, Dropout 1,
and Reshape 1. A downloader must verify the exact hash above; tests never
silently accept a moving model URL.

Primary sources:

- <https://github.com/onnx/models/tree/main/validated/vision/classification/squeezenet>
- <https://huggingface.co/onnxmodelzoo/squeezenet1.1-7/blob/main/squeezenet1.1-7.onnx>
- <https://onnx.ai/onnx/repo-docs/IR.html>

## Import semantics

The FLOAT profile supports one static, dense, float32 inference graph. It rejects
unknown domains, opsets other than 7, subgraphs, fns, sparse values,
external tensor data, symbolic or absent dimensions, multiple graph outputs,
and unsupported attributes before publishing a Mod.

IR 3 lists initializers among graph inputs. This importer intentionally freezes
every initializer; only graph inputs without a matching initializer become
Fn arguments. Each initializer is decoded to canonical element bytes,
stored with `Mod::store`, and materialized through
`onnx.Constant(content: string)`. The original ONNX bytes are also retained
as immutable Mod data.

The model contains no intermediate type information, so the importer performs
checked static shape propagation for its supported operators. It verifies the
inferred output against the declared graph output before committing the
Fn.

The current native importer still contains per-operator validation and shape
propagation. This is retained only as verified baseline behavior while it is
being replaced by schema-driven binding; it is not the accepted extension
architecture.

Current mapping behavior is:

- Conv verifies `kernel_shape` against the weight Type, then records strides,
  pads, dilations, and group in the tensor call;
- Relu, MaxPool, AveragePool, and two-input Concat map directly;
- Dropout-7 with one output is an identity in inference mode and adds no tensor
  call;
- Reshape-5 requires a constant int64 shape input, resolves `0` and one `-1`
  for its result Type, and retains the original shape list as a binding of the
  specialized callee value.

## Provenance

Frontend names are diagnostic provenance, not tensor semantics. `Op` exposes
its existing optional `Loc`, and `Fn::Edit` may attach one. The
ONNX importer uses the source string for graph, node, and output names and the
range coordinates for deterministic node ordinals. Cloning and direct Fn
inlining preserve source locations. No `name` parameter is added to every
tensor fn.

Locations are excluded from canonical semantic formatting and Mod identity.
The original ONNX payload remains the lossless source record. Repository and
external CLI bundles preserve Mod-owned binary data through Design 0006, so the
reference model is installable through the public workflow.

## Dependency boundary

Compiler core remains dependency-free. The optional ONNX native Mod uses
the system Protobuf runtime and generates bindings from a vendored, hash-pinned
official `onnx.proto`; generated C++ is build output, not checked-in API. A
build without Protobuf still builds and installs Joggle core and tensor.

## Implementation gates

- [x] Expose and preserve ordinary Op source locations through public editing,
  cloning, and direct Fn inlining.
- [x] Add the `onnx` source Mod, pinned schema provenance, optional Protobuf
  native target, and hash-checking model fetch command.
- [x] Parse and reject unsupported ModelProto structure transactionally.
- [x] Import the exact reference model with one input, one output, 52 tensor
  constants, 65 semantic calls, and output Type `f32[1,1000]`.
- [x] Verify initializer digests, callee bindings, every propagated shape,
  source locations, and deterministic failure diagnostics.
- [x] Differentially validate deterministic inputs against ONNX Runtime before
  calling the path an ONNX frontend.
- [x] Move model constants and ONNX operator declarations out of `tensor`.
- [x] Expand every Relu first through bodyful `map`, then through `build/at`,
  using generic `transform.inline` without a Conv/Relu matcher.
- [ ] Replace the fixed importer Schema and `op_type` branch chain with generic
  binding against ONNX Mod fn signatures.
- [ ] Move output-type and shape computation into explicitly staged source fns.
- [ ] Give supported compute operators inspectable tensor-calculus bodies.
