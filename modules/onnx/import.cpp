...
struct Schema {
  ...
  joggle::Module::FunctionDecl softmax;
  std::optional<joggle::Module::FunctionDecl> quantize;
  std::optional<joggle::Module::FunctionDecl> dequantize;
...
std::optional<Schema>
load_schema(joggle::Compiler& compiler, bool needs_quant,
            joggle::Diagnostics& diagnostics) {
  const auto module = compiler.module("tensor");
  const auto quant = compiler.module("quant");
  ...
  const auto integer_list =
      integer && integer_list_decl
          ...
          : std::nullopt;
  if (!module || !integer || !boolean || !string || !integer_list) {
    fail(diagnostics, "linked tensor and Prelude schemas are unavailable");
    return std::nullopt;
  }
  if (needs_quant && !quant) {
    fail(diagnostics, "the QDQ profile requires the quant Module");
    return std::nullopt;
  }
  ...
  const auto softmax = overload(*module, "softmax", 2);
  ...
  if (!tensor || !constant || !conv || !conv_bias || !relu || !max_pool ||
    ...
    fail(diagnostics,
         "tensor or quant Module declarations do not match ONNX importer");
    return std::nullopt;
  }
  std::map<std::int32_t, joggle::Type> elements;
  ...
  return Schema{*tensor,       *constant,    *conv,
                *conv_bias,    *relu,        *max_pool,
                *average_pool, *concat,      *reshape,
                *softmax,      quantize,     dequantize,
                std::move(elements), *integer, *boolean,
                *string,       *integer_list};
}

std::optional<joggle::Type>
tensor_type(joggle::Compiler& compiler, const Schema& schema,
...
string_attribute(const std::map<std::string, const onnx::AttributeProto*>& attrs,
...
profile(const onnx::ModelProto& source, joggle::Diagnostics& diagnostics) {
...
read(joggle::Compiler& compiler, const joggle::Bytes& input, std::string name,
  ...
  if (*selected == Profile::Qdq13) {
    const bool has_quantize =
        std::any_of(graph.node().begin(), graph.node().end(),
                    [](const auto& node) {
                      return node.op_type() == "QuantizeLinear";
                    });
    const bool has_dequantize =
        std::any_of(graph.node().begin(), graph.node().end(),
                    [](const auto& node) {
  ...
  const auto declare = [&](const onnx::ValueInfoProto& value) {
    ...
    return declared &&
           declared_values.emplace(value.name(), *declared).second;
  };
  ...
  for (const auto& input_value : graph.input()) {
    ...
    values.emplace(input_value.name(),
                   Tensor{edit.argument(*type), declared.first,
                          declared.second});
  }
  ...
  for (const auto& tensor : graph.initializer()) {
    ...
    const auto type = tensor_type(compiler, *schema, initializer.element,
                                  initializer.shape);
    ...
    values.emplace(tensor.name(),
                   Tensor{constant.value(), initializer.element,
                          initializer.shape});
    ++initializer_ordinal;
  }
  ...
  const std::set<std::string_view> legacy_ops{
      "AveragePool", "Concat", "Conv", "Dropout", "MaxPool", "Relu",
      "Reshape", "Softmax"};
  const std::set<std::string_view> qdq_ops{
      "Concat",          "Conv",              "DequantizeLinear",
      "Flatten",         "GlobalAveragePool", "MaxPool",
      "QuantizeLinear",  "Reshape",           "Softmax"};
  ...
  for (std::size_t node_index = 0;
    ...
    if (node.op_type() == "QuantizeLinear" ||
      ...
      const auto input_tensor =
          node.input_size() == 3 ? require_value(node.input(0))
                                 : std::optional<Tensor>{};
      ...
      const auto zero = node.input_size() == 3 ? require_value(node.input(2))
                                               : std::optional<Tensor>{};
      const auto axis =
          attrs ? integer_attribute(*attrs, "axis", 1, diagnostics)
                : std::nullopt;
      ...
      if ((!scalar_parameters && !per_axis_parameters) || !storage_type ||
        ...
        fail(diagnostics, node.op_type() +
                              " has inconsistent tensor or parameter Types");
        return std::nullopt;
      }
      const auto axis_value = known(compiler, schema->integer, *axis);
      output_element = quantizing ? zero->element
                                  : onnx::TensorProto_DataType_FLOAT;
      output_shape = input_tensor->shape;
      const auto declaration =
          quantizing ? schema->quantize : schema->dequantize;
      if (!axis_value || !declaration) {
        fail(diagnostics, "quant Module declarations are unavailable");
        return std::nullopt;
      }
      call = append_call(*declaration,
                         {input_tensor->value, scale->value, zero->value,
                          *axis_value},
      ...
      const bool channel_product_overflows =
          ...
          *group > std::numeric_limits<std::int64_t>::max() /
                       weight->shape[1];
      if (!strides || !pads || !dilations || !kernel || !group ||
          !auto_pad || *auto_pad != "NOTSET" ||
      ...
      call = append_call(bias ? schema->conv_bias : schema->conv,
                         std::move(arguments),
                         onnx::TensorProto_DataType_FLOAT, output_shape);
      ...
      const auto attrs = attributes(
          ...
          average ? std::set<std::string>{"ceil_mode", "count_include_pad",
                                          ...
                                          "strides"}
                  : std::set<std::string>{"ceil_mode", "auto_pad",
                                          "dilations", "kernel_shape", "pads",
      ...
      const auto outer = element_count(std::span<const std::int64_t>(
          input_tensor->shape.data(), split));
      const auto inner = element_count(std::span<const std::int64_t>(
          input_tensor->shape.data() + split,
      ...
      call = append_call(schema->reshape,
                         {input_tensor->value, *shape_value}, output_element,
                         output_shape);
      ...
      call = append_call(schema->concat,
                         {lhs->value, rhs->value, *axis_value}, lhs->element,
                         output_shape);
