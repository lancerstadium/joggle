#pragma once

#include <cstdint>
#include <iterator>
#include <memory>
#include <optional>
#include <span>
#include <string>
#include <string_view>
#include <type_traits>
#include <utility>
#include <vector>

#include "joggle/mod.h"

namespace joggle {

class Compiler;

namespace detail {
class ParamVal;
struct TypeStorage;
struct ParamValStorage;
struct TypeAccess;
template <typename T, typename Val>
std::optional<T> get_parameter(const Val& value, std::string_view name);
}  // namespace detail

class Type {
public:
  Mod::TypeDecl schema() const;
  std::string_view stable_name() const;

  template <typename T> std::optional<T> get(std::string_view name) const;

  bool operator==(const Type& other) const;

private:
  std::span<const detail::ParamVal> parameters() const;
  std::span<const detail::ParamVal> derived_parameters() const;
  explicit Type(std::shared_ptr<const detail::TypeStorage> storage);
  std::shared_ptr<const detail::TypeStorage> storage_;

  friend class Compiler;
  friend class detail::ParamVal;
  friend struct detail::TypeAccess;
  template <typename T, typename Val>
  friend std::optional<T> detail::get_parameter(const Val&, std::string_view);
};

namespace detail {

class ParamVal {
public:
  enum class Kind { I64, F64, Boolean, String, Type, List };

  ParamVal(std::int64_t value);
  ParamVal(double value);
  ParamVal(bool value);
  ParamVal(std::string value);
  ParamVal(const char* value);
  ParamVal(Type value);

  static ParamVal list(std::vector<ParamVal> values);

  Kind kind() const;
  std::span<const ParamVal> elements() const;
  const std::int64_t* as_i64() const;
  const double* as_f64() const;
  const bool* as_bool() const;
  const std::string* as_string() const;
  const Type* as_type() const;
  std::string canonical() const;
  bool operator==(const ParamVal& other) const;

private:
  explicit ParamVal(std::shared_ptr<const ParamValStorage> storage);
  std::shared_ptr<const ParamValStorage> storage_;

  friend class Compiler;
};

template <typename> inline constexpr bool always_false = false;

template <typename> struct VectorElement;

template <typename T, typename Allocator>
struct VectorElement<std::vector<T, Allocator>> {
  using type = T;
};

template <typename T> struct IsVector : std::false_type {};

template <typename T, typename Allocator>
struct IsVector<std::vector<T, Allocator>> : std::true_type {};

template <typename T> inline constexpr bool is_vector = IsVector<T>::value;

template <typename T, typename = void> struct IsRange : std::false_type {};

template <typename T>
struct IsRange<T, std::void_t<decltype(std::begin(std::declval<T&>())),
                              decltype(std::end(std::declval<T&>()))>>
    : std::true_type {};

template <typename T> inline constexpr bool is_range = IsRange<T>::value;

template <typename T> std::optional<T> decode_parameter(const ParamVal& value) {
  if constexpr (std::is_same_v<T, std::int64_t>) {
    const auto* decoded = value.as_i64();
    return decoded ? std::optional<T>{*decoded} : std::nullopt;
  } else if constexpr (std::is_same_v<T, double>) {
    const auto* decoded = value.as_f64();
    return decoded ? std::optional<T>{*decoded} : std::nullopt;
  } else if constexpr (std::is_same_v<T, bool>) {
    const auto* decoded = value.as_bool();
    return decoded ? std::optional<T>{*decoded} : std::nullopt;
  } else if constexpr (std::is_same_v<T, std::string>) {
    const auto* decoded = value.as_string();
    return decoded ? std::optional<T>{*decoded} : std::nullopt;
  } else if constexpr (std::is_same_v<T, Type>) {
    const auto* decoded = value.as_type();
    return decoded ? std::optional<T>{*decoded} : std::nullopt;
  } else if constexpr (is_vector<T>) {
    using Element = typename VectorElement<T>::type;
    if (value.kind() != ParamVal::Kind::List) {
      return std::nullopt;
    }
    T decoded;
    decoded.reserve(value.elements().size());
    for (const ParamVal& item : value.elements()) {
      auto element = decode_parameter<Element>(item);
      if (!element) {
        return std::nullopt;
      }
      decoded.push_back(std::move(*element));
    }
    return decoded;
  } else {
    static_assert(always_false<T>, "unsupported Joggle parameter type");
  }
}

template <typename T> Mod::ParamDecl cpp_parameter() {
  if constexpr (std::is_same_v<T, std::int64_t>) {
    return {"", Mod::Expr::reference("int"), false, std::nullopt};
  } else if constexpr (std::is_same_v<T, double>) {
    return {"", Mod::Expr::reference("real"), false, std::nullopt};
  } else if constexpr (std::is_same_v<T, bool>) {
    return {"", Mod::Expr::reference("bool"), false, std::nullopt};
  } else if constexpr (std::is_same_v<T, std::string>) {
    return {"", Mod::Expr::reference("string"), false, std::nullopt};
  } else if constexpr (std::is_same_v<T, Type>) {
    return {"", Mod::Expr::reference("type"), false, std::nullopt};
  } else if constexpr (is_vector<T>) {
    auto result = cpp_parameter<typename VectorElement<T>::type>();
    result.domain = Mod::Expr::list_domain(std::move(result.domain));
    return result;
  } else {
    static_assert(always_false<T>, "unsupported Joggle method type");
  }
}

template <typename T> ParamVal encode_parameter(T&& value) {
  using Stored = std::remove_cvref_t<T>;
  if constexpr (std::is_same_v<Stored, ParamVal>) {
    return std::forward<T>(value);
  } else if constexpr (std::is_same_v<Stored, bool>) {
    return ParamVal(value);
  } else if constexpr (std::is_integral_v<Stored>) {
    return ParamVal(static_cast<std::int64_t>(value));
  } else if constexpr (std::is_floating_point_v<Stored>) {
    return ParamVal(static_cast<double>(value));
  } else if constexpr (std::is_same_v<Stored, std::string>) {
    return ParamVal(std::forward<T>(value));
  } else if constexpr (std::is_convertible_v<T, std::string_view>) {
    return ParamVal(std::string(std::string_view(value)));
  } else if constexpr (std::is_same_v<Stored, Type>) {
    return ParamVal(std::forward<T>(value));
  } else if constexpr (std::is_same_v<Stored, std::vector<bool>>) {
    std::vector<ParamVal> elements;
    elements.reserve(value.size());
    for (const bool item : value) {
      elements.emplace_back(item);
    }
    return ParamVal::list(std::move(elements));
  } else if constexpr (is_range<Stored>) {
    std::vector<ParamVal> elements;
    for (auto&& item : value) {
      elements.push_back(encode_parameter(std::forward<decltype(item)>(item)));
    }
    return ParamVal::list(std::move(elements));
  } else {
    static_assert(always_false<Stored>, "unsupported Joggle parameter type");
  }
}

template <typename T, typename Val>
std::optional<T> get_parameter(const Val& value, std::string_view name) {
  const auto schema = value.schema().parameters();
  const auto parameters = value.parameters();
  for (std::size_t index = 0; index < schema.size(); ++index) {
    if (schema[index].name == name) {
      return index < parameters.size() ? decode_parameter<T>(parameters[index])
                                       : std::nullopt;
    }
  }
  if constexpr (std::is_same_v<Val, Type>) {
    const auto derived_schema = value.schema().derived_parameters();
    const auto derived = value.derived_parameters();
    for (std::size_t index = 0; index < derived_schema.size(); ++index) {
      if (derived_schema[index].name == name) {
        return index < derived.size() ? decode_parameter<T>(derived[index])
                                      : std::nullopt;
      }
    }
  }
  return std::nullopt;
}

}  // namespace detail

template <typename T> std::optional<T> Type::get(std::string_view name) const {
  return detail::get_parameter<T>(*this, name);
}

}  // namespace joggle
