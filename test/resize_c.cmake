if(NOT DEFINED TOOL OR NOT DEFINED CC OR NOT DEFINED MODEL OR
  ...
  message(FATAL_ERROR
          "Resize C test requires TOOL, CC, MODEL, HARNESS, MODULES, ROOT")
endif()
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR "Resize preparation failed (${result}):\n${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR "Resize planning failed (${result}):\n${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0 OR NOT frontier STREQUAL "[]\n")
  message(FATAL_ERROR
          "prepared Resize retains a C frontier (${result}):\n${frontier}${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR "Resize emission failed (${result}):\n${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR
          "Resize C did not compile (${result}):\n${output}${error}")
endif()
execute_process(
  COMMAND "${program}"
  RESULT_VARIABLE result OUTPUT_VARIABLE output ERROR_VARIABLE error
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR
          "Resize C returned wrong values (${result}):\n${output}${error}")
endif()
