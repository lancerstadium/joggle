if(NOT DEFINED TOOL OR NOT DEFINED CC OR NOT DEFINED MODEL OR
  ...
  message(FATAL_ERROR
          "Range C test requires TOOL, CC, MODEL, HARNESS, MODULES, ROOT")
endif()
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR "Range preparation failed (${result}):\n${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR "Range planning failed (${result}):\n${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0 OR
   NOT api MATCHES "\\\"capacity\\\": \\[4\\]" OR
   NOT api MATCHES "\\\"shape\\\": \\[\\\"_\\\"\\]")
  message(FATAL_ERROR
          "Range C API lacks its shape contract (${result}):\n${api}${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0 OR NOT frontier STREQUAL "[]\n")
  message(FATAL_ERROR
          "prepared Range retains a C frontier (${result}):\n${frontier}${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR "Range emission failed (${result}):\n${error}")
endif()
execute_process(
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR
          "Range C did not compile (${result}):\n${output}${error}")
endif()
execute_process(
  COMMAND "${program}"
  RESULT_VARIABLE result OUTPUT_VARIABLE output ERROR_VARIABLE error
...
if(NOT result EQUAL 0)
  message(FATAL_ERROR
          "Range C returned wrong values (${result}):\n${output}${error}")
endif()
