...
constexpr std::int32_t onnx_float = 1;
...
bool tensor(std::string_view path, Tensor& tensor) {
  ...
  if (!value.ParseFromArray(encoded.data(), static_cast<int>(encoded.size())) ||
      value.data_type() != onnx_float || value.dims().empty() ||
      !value.has_raw_data())
    return false;
  std::size_t count = 1;
  ...
  if (count > std::numeric_limits<std::size_t>::max() / sizeof(float) ||
    ...
    return false;
  tensor.shape.assign(value.dims().begin(), value.dims().end());
  tensor.data.assign(value.raw_data().begin(), value.raw_data().end());
  return true;
}
...
bool close(const Bytes& actual, const Bytes& expected) {
  ...
  for (std::size_t offset = 0; offset < actual.size();
    ...
    if (!std::isfinite(value) || !std::isfinite(reference) ||
        std::fabs(value - reference) > 1.0e-5F + 1.0e-5F * std::fabs(reference))
      return false;
  }
  return true;
}
...
std::string harness(const Tensor& left, const Tensor& right,
                    const Tensor& expected) {
  const std::size_t left_count = left.data.size() / sizeof(float);
  const std::size_t right_count = right.data.size() / sizeof(float);
  const std::size_t output_count = expected.data.size() / sizeof(float);
  std::ostringstream out;
  out << "#include <math.h>\n"
         "#include <stddef.h>\n"
         "#include <string.h>\n"
         "void model_main(const float*, const float*, float*);\n"
      << array("left_bytes", left.data) << array("right_bytes", right.data)
      << array("expected_bytes", expected.data)
      << "int main(void) {\n"
         "  float left["
      << left_count << "], right[" << right_count << "], expected["
...
int main(int argc, char** argv) {
  ...
  Tensor right;
  Tensor expected;
  CHECK(tensor(argv[2], left));
  CHECK(tensor(argv[3], right));
  CHECK(tensor(argv[4], expected));
  CHECK(left.shape.size() == 2 && right.shape.size() == 2 &&
        expected.shape.size() == 2 && left.shape[1] == right.shape[0] &&
        expected.shape[0] == left.shape[0] &&
        expected.shape[1] == right.shape[1]);
  ...
  const std::vector<joggle::Attr> dce_args{joggle::Attr(joggle::Attr::List{})};
  ...
  CHECK(model.verify(env));
  const std::string loop = "for i in 0.." + std::to_string(left.shape[0]) +
                           ", k in 0.." + std::to_string(left.shape[1]) +
                           ", j in 0.." + std::to_string(right.shape[1]);
  CHECK(joggle::print(model).find(loop) != std::string::npos);
  ...
  CHECK(image.string());
  Bytes input = left.data;
  input.insert(input.end(), right.data.begin(), right.data.end());
  ...
  CHECK(vm_result.size() == 2 && vm_result[0].bytes() &&
        close(*vm_result[0].bytes(), expected.data) && vm_result[1].integer() &&
  ...
  CHECK(c_source.string() &&
        c_source.string()->find("onnx") == std::string::npos);
