...
std::string sha256(std::string_view input) {
  ...
  for (int shift = 56; shift >= 0; shift -= 8) {
    bytes.push_back(static_cast<std::uint8_t>((bit_count >> shift) & 0xffU));
  }
  ...
  for (std::size_t offset = 0; offset < bytes.size(); offset += 64U) {
    ...
    for (std::size_t index = 16U; index < words.size(); ++index) {
      const std::uint32_t small0 = rotate_right(words[index - 15U], 7U) ^
                                   rotate_right(words[index - 15U], 18U) ^
                                   (words[index - 15U] >> 3U);
      const std::uint32_t small1 = rotate_right(words[index - 2U], 17U) ^
    ...
    for (std::size_t index = 0; index < words.size(); ++index) {
      ...
      const std::uint32_t temporary1 =
          h + large1 + choose + round_constants[index] + words[index];
