...
std::optional<Storage> storage(const joggle::Type& type, bool quantizing,
  ...
  fail(diagnostics, quantizing ? "quantize storage must be u8 or i8"
...
std::optional<std::size_t> element_count(const Shape& shape,
  ...
  for (const auto dimension : shape) {
    ...
    if (extent > std::numeric_limits<std::size_t>::max() ||
        (extent != 0U && count > std::numeric_limits<std::size_t>::max() /
...
struct Parameters {
  ...
  std::size_t at(std::size_t linear) const {
    return scales.size() == 1U ? 0U : (linear / axis_stride) % axis_extent;
  }
};
...
std::optional<Parameters> parameters(const Reals& scales, const Integers& zeros,
  ...
  for (const double value : scales) {
    ...
    if (!std::isfinite(value) || !std::isfinite(narrowed) || narrowed <= 0.0F) {
  ...
  if (normalized < 0 || normalized >= static_cast<std::int64_t>(shape.size())) {
  ...
  for (std::size_t dimension = index + 1U; dimension < shape.size();
    ...
    if (extent != 0U &&
        result.axis_stride > std::numeric_limits<std::size_t>::max() / extent) {
...
bool zeros_fit(const Integers& zeros, Storage format, bool quantizing,
  ...
  if (!std::all_of(zeros.begin(), zeros.end(), in_range)) {
    fail(diagnostics, format == Storage::I32 && !quantizing
...
quantize(const joggle::Bytes& input, const Reals& scales, const Integers& zeros,
  ...
  if (!format || !expected || !affine || input.size() != *expected ||
    ...
    if (format && expected && affine && input.size() != *expected &&
        diagnostics.ok()) {
      fail(diagnostics,
           "quantize input is not one little-endian f32 per element");
    }
    return std::nullopt;
  }
  ...
  for (std::size_t index = 0; index < *elements; ++index) {
    ...
    const auto bits =
        *format == Storage::U8
            ...
            : std::bit_cast<std::uint8_t>(static_cast<std::int8_t>(result));
...
std::int64_t read_integer(const joggle::Bytes& input, std::size_t index,
  ...
  for (std::size_t byte = 0; byte < 4U; ++byte) {
    bits |= static_cast<std::uint32_t>(
                std::to_integer<unsigned char>(input[index * 4U + byte]))
            << (byte * 8U);
  }
