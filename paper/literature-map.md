...
| Q | Glow: Graph Lowering Compiler Techniques for Neural Networks | C | Strong graph lowering and instruction IR; contrasts staged representations |
| Q | nGraph: a New Compiler for Deep Learning Frameworks | C | Framework-neutral graph compilation and backend interfaces |
| Q | Tensor Comprehensions: Framework-Agnostic High-Performance Machine Learning Abstractions | M | Concise semantics plus polyhedral/code-generation path |
| Q | Relay: A High-Level Compiler for Deep Learning | M | Functional graph IR, type/shape reasoning, and compiler composition |
| Q | Relax: Composable Abstractions for End-to-End Dynamic Machine Learning | M | Dynamic shape and cross-level composition in modern TVM |
| Q | MNN: A Universal and Efficient Inference Engine | E | Lightweight mobile runtime and model-conversion ecosystem |
...
| Q | Tiramisu: A Polyhedral Compiler for Expressing Fast and Portable Code | M | Explicit schedule commands over affine computations |
...
| Q | TensorIR: An Abstraction for Automatic Tensorized Program Optimization | M | Schedulable tensor programs and tensorization boundaries |
| Q | Triton: An Intermediate Language and Compiler for Tiled Neural Network Computations | M | Programmable tiled GPU kernels |
...
| Q | egg: Fast and Extensible Equality Saturation | C | General rewrite infrastructure used by tensor optimizers |
| Q | BOLT: Bridging the Gap between Auto-tuners and Hardware-native Performance | M | Fast generated kernels versus vendor libraries |
| Q | AStitch: Enabling a New Multi-dimensional Optimization Space for Memory-Intensive ML | M | Whole-subgraph fusion and memory scheduling |
...
| Q | Nimble: Efficiently Compiling Dynamic Neural Networks for Model Inference | M | Ahead-of-time planning under input-dependent execution |
| Q | DISC: A Dynamic Shape Compiler for Machine Learning Workloads | M | Dynamic shape specialization and production deployment |
| Q | DietCode: Automatic Optimization for Dynamic Tensor Programs | M | Schedule reuse across dynamic shapes |
| Q | Cortex: A Compiler for Recursive Deep Learning Models | C | Model structure and runtime dynamism beyond static DAGs |
...
| Q | DNNFusion: Accelerating Deep Neural Networks Execution with Advanced Operator Fusion | M | Graph fusion profitability and generated kernels |
...
| Q | MCUNet: Tiny Deep Learning on IoT Devices | E | Network/runtime co-design under SRAM and flash constraints |
| Q | TinyEngine: A Memory-Efficient and High-Performance Neural Network Library for Microcontrollers | E | In-place depthwise execution and memory scheduling |
| Q | DORY: Automatic End-to-End Deployment of Real-World DNNs on Low-Cost IoT MCUs | M | Deployment, tiling, and heterogeneous memory |
| Q | PULP-NN: Accelerating Quantized Neural Networks on Parallel Ultra-Low-Power RISC-V Processors | M | Low-bit kernels and ISA-aware deployment |
...
| Q | VTA: An Open Hardware-Software Stack for Deep Learning | M | Extensible accelerator ISA and compiler co-design |
| Q | Timeloop: A Systematic Approach to DNN Accelerator Evaluation | C | Explicit mapping-space and hardware cost modeling |
| Q | MAESTRO: A Data-Centric Approach to Understand Reuse, Performance, and Hardware Cost of DNN Mappings | C | Dataflow cost models suitable for optional policy modules |
| Q | Interstellar: Using Halide's Scheduling Language to Analyze DNN Accelerators | C | One schedule notation spanning algorithms and accelerators |
...
| Q | HirGen: Generating Efficient and High-Coverage Tests for Deep Learning Library Compilers | M | Hierarchical IR fuzzing and coverage |
| Q | An Empirical Study of Bugs in Deep Learning Compilers | C | Failure taxonomy and the cost of complex lowering stacks |
| Q | MT-DLComp: Metamorphic Testing for Deep Learning Compilers | C | Oracle-free differential properties |

### G. Workloads that must bound the evaluation

| State | Work | Role | Why it matters |
| --- | --- | --- | --- |
| Q | Attention Is All You Need | C | Establishes attention's contraction, normalization, and shape patterns |
| Q | BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding | C | Encoder workload with embeddings, normalization, attention, and dynamic sequence axes |
| Q | An Image Is Worth 16x16 Words: Transformers for Image Recognition at Scale | C | ViT adds attention to a conventional static vision task |
...
The pool currently contains 64 candidate works: 18 primary-record checks and
46 queued checks. The counts are intentionally visible so a partially verified
