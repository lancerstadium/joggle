...
\begin{figure}[t]
  \centering
  \includegraphics[width=.80\columnwidth]{figures/figure-02-system-model.png}
  \caption{A typed compiler call executes transactionally and records reads
    and effects for reuse.}
  \Description{A typed function enters an evaluator that reads and writes a
    subject graph under a transaction. A compact record stores the call key,
    result, observed reads, and effects; current records return to the
    evaluator as cache hits.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=.80\columnwidth]{figures/figure-03-dependencies.png}
  \caption{Static \texttt{use} edges bound calls; dynamic read/effect records
    select work after an edit.}
  \Description{The upper region shows the static use graph of seven mods. The
    lower ledger shows each stage's observed reads, effects, and selection
    decision after a value-type edit.}
