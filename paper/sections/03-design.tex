...
A common operator extension illustrates these roles. A convolution, bias
addition, and activation form a fusible subgraph. One mod owns the fused
...
For the operator example above, a type edit directly invalidates the legality
stage that observed it. The fusion stage follows when its recorded input
overlaps the earlier effect, while stages with disjoint observations retain
