...
Figure~\ref{fig:system-model} follows this call from definition to publication.
A mod supplies the typed function, the evaluator observes its graph access,
and the runtime either commits a verified update or returns a read-only result.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/figure-02-system-model.png}
  \caption{Joggle expresses compiler behavior as typed functions, organizes
    them in mods, and executes them over a transactional graph runtime.}
  \Description{An extension mod and typed function enter an evaluator. The
    evaluator accesses a subject store, records reads and effects, verifies
    mutations in a transaction, and returns a result or artifact. Reusable
    dependency records feed back to the evaluator.}
  \label{fig:system-model}
\end{figure}

The remainder of this section develops the model in execution order.
Section~\ref{sec:compiler-functions} defines compiler functions.
Section~\ref{sec:mods} explains how mods own and compose them.
Section~\ref{sec:incremental} derives incremental execution from observed
dependencies. Section~\ref{sec:graph-runtime} presents the runtime mechanisms
that preserve these contracts.
...
Listing~\ref{lst:sat} shows these roles in the bundled \texttt{sat} mod. The
same syntax declares a parametric type, a capability query, native simulation
and artifact functions, and a mutating selection stage.

\begin{lstlisting}[language=jog,caption={The \texttt{sat} mod defines semantics,
analysis, native boundaries, and transformation with one function syntax.},
label={lst:sat}]
mod sat
use ir
...
Given a \texttt{sat<W>} result, \texttt{select} calls \texttt{supports},
retargets the addition to \texttt{sat.add}, and reports whether the subject
changed. Its signature distinguishes mutation from the read-only query.
Body-less \texttt{sim} and \texttt{emit} declarations bind native
implementations behind the same typed call boundary.
...
be a source body, an intrinsic, or a native binding. This choice does not change
the caller's syntax or the evaluator's result boundary.
...
Compiler functions also compose through ordinary calls. In
Listing~\ref{lst:sat}, \texttt{select} invokes \texttt{sat.supports}; larger
policies can call analyses, converters, selectors, and planners from their
declared dependencies. The call graph is therefore available to type checking,
diagnostics, and dependency capture. It does not need a second, global pipeline
registry.
...
on-disk package is a distribution form of this object, not its semantic
boundary.
...
mod, rather than a collection of edits to global registries, the unit that
evolves.
...
Static mod dependencies remain deliberately coarse. They state what a compiler
extension may call. During execution, the evaluator records which functions and
...
\[
  \operatorname{Selected}_i = \neg \operatorname{Current}(D_i) \lor
  \exists j<i:\ \operatorname{Selected}_j \land
...
Figure~\ref{fig:dependencies} separates the dependency levels. The static
\texttt{use} graph defines which functions may be called; a compact
observation ledger records which entities a particular run read. An edit
invalidates the stage that observed it and only the later stages whose
recorded inputs overlap its effects.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/figure-03-dependencies.png}
  \caption{Static \texttt{use} edges define available compiler capabilities,
    whereas dynamic observations determine the work selected after an edit.}
  \Description{The upper panel shows the static use graph of seven mods. The
    lower panel shows a value-type edit directly selecting one stage,
    propagating to one downstream stage, and preserving two reusable stages.}
...
\[
  \begin{aligned}
    T_{\mathrm{select}}
      &= \sum_{i=1}^{K} T_{\mathrm{validate}}(D_i)
         + T_{\mathrm{propagate}}, \\
    T_{\mathrm{inc}}
      &= T_{\mathrm{select}} + T_{\mathrm{commit}} \\
      &\quad + \sum_{s_i \in \mathrm{Selected}}
        \left(T_{\mathrm{eval}}(s_i) + T_{\mathrm{verify}}(s_i)\right).
  \end{aligned}
\]
...
reads through the public API. These rules define the state over which Joggle can
validate reuse.
...
\[
  ...
  \begin{cases}
    (G', \mathrm{ok}) &
      \text{if } \operatorname{Eval}(f,G) \Downarrow G'
...
plan maps parameters, block arguments, and operation results to dense slots; it
...
native machine code. They remove repeated interpretation setup while preserving
the language's control flow, failure propagation, and dynamic calls. A native
implementation may accelerate a specific typed function, but it enters through
the same mod-scoped binding and graph API.
