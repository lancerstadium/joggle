...
A second regression copies the actual \texttt{c.prepare} function together
with its transitive private helper closure. The copied private
\texttt{prepare\_with} body is edited so its second \texttt{opt.expose} call
returns \texttt{false}; the code module is verified. On one simple typed
model, both the original and the derived procedure produce byte-identical
prepared IR and generated C source, and the original remains callable after
the edit. This is the first checked edit to a real compiler procedure, but a
single model cannot establish the rewrite's general legality or runtime
benefit. The larger-model and cost comparison remains open.
