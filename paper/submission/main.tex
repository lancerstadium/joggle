...
Our S0 preflight makes the extension contracts concrete. Joggle's out-of-tree
\texttt{projection} module defines the Conv candidate, its selection, and its
external declaration; the existing C path closes the artifact. TVM's authored
Relax mutator selects a TensorIR external body, and its artifact builder links
the supplied object through \texttt{export\_library}. Pinned ONNX-MLIR uses its
documented Conv call route, an OMTensor adapter, and a core conversion fix for
integer-list attributes. All three produce oracle-checked artifacts. A file
inside framework core is not automatically a registration boundary, just as a
single Python script is not one semantic contract. Only the later S1/S2
revisions can measure whether these interfaces require repeated coordination.
