...
\section{Motivation: the vertical slice is the experiment}
\subsection{A co-design change crosses compiler boundaries}
...
researcher, however, the unit of experimentation becomes a \emph{vertical slice}
...
new mechanism an \textbf{extension discontinuity}. The claim is not that every
...
\subsection{Control and automation solve different halves}
Kernel languages give authors precise control at a lower level. \cite{ragankelley2013halide}
...
Automatic systems attack the authoring burden from the other side. \cite{zheng2020ansor}
search explores tensor programs, while \cite{wu2024mirage} searches across graph, block, and
thread levels. \cite{kothari2026axon} goes further: it synthesizes target instructions from
...
\subsection{Edge deployment makes the unsupported path visible}
...
\begin{itemize}
\item
\textbf{RQ1, continuity:} Can conventional CNN, detection, ViT/attention, and
...
\item
\textbf{RQ2, extension boundary:} On matched tasks, how many distinct definitions,
...
\item
\textbf{RQ3, composition:} Do independently installed modules compose with stable
output, transactional failure, and useful unsupported-frontier diagnostics?

\item
\textbf{RQ4, artifact quality:} Under matched model, host, threading, and numerical
...
\centering
\scriptsize
\caption{Pinned ONNX Model Zoo structural frontier}
\label{tab:pinned-onnx-model-zoo-structural-frontier}
...
tiny-yolov3-11 & 291 & partial (219 unknown) & not\_run \\
...
\centering
\scriptsize
\caption{Frozen extension tasks and observed authored source surface}
\label{tab:frozen-extension-tasks-and-observed-authored-source-surface}
\begin{tabularx}{\textwidth}{@{}lXXX@{}}
\toprule
...
ONNX fixture with its exact \nolinkurl{2x3} and \nolinkurl{3x2} inputs. Joggle imports that model,
expands the ordinary \nolinkurl{ikj} function, and reproduces the same TensorProto oracle
through both its deterministic VM and strictly compiled C. The same model now
passes a native ONNX-MLIR \nolinkurl{0.4.2} extension at pinned revision \nolinkurl{4a13c34a} and
its documented LLVM revision. The preserved lowered IR contains explicit
\nolinkurl{i-k-j} affine loops, and its shared library returns \nolinkurl{[58, 64, 139, 154]} with
zero maximum absolute error.

The first three frozen contracts now also pass on a pinned TVM \nolinkurl{v0.26.0}
...
the corresponding source module is absent, and parsing \nolinkurl{custom[sat]5} reports
that \nolinkurl{dtype.get_custom_type_code} is unavailable. We therefore preserve an
...
the ONNX-MLIR driver describes \nolinkurl{--ops-for-call=Conv MatMul} as its example,
running \nolinkurl{--ops-for-call=MatMul} on the frozen \nolinkurl{2x3} by \nolinkurl{3x2} input emits three
ordinary affine loops and no \nolinkurl{krnl.call}. At the pinned revision, Conv registers
...
numeric-format accelerator defines one parameterized \nolinkurl{!sat.int<W>} type,
...
artifact returns \nolinkurl{[15]}, \nolinkurl{[-16]}, \nolinkurl{[127]}, \nolinkurl{[2047]}, and
\nolinkurl{[15, 15, -16, -16]} exactly; invalid widths, mixed widths, missing
...
\cite{jin2020onnxmlir}, exposes documented
operation-generation and accelerator integration paths, and produces native
artifacts (\href{https://onnx.ai/onnx-mlir/}{official documentation}). It is
...
configurations
\cite{liu2022tinyiree}. Joggle does not
...
\cite{wang2025tilelang}. These systems are the
...
\cite{steuwer2022rise}. Joggle shares their
...
(\href{https://github.com/Tencent/ncnn}{project},
\href{https://github.com/Tencent/ncnn/wiki/how-to-implement-custom-layer-step-by-step}{extension guide}).
