...
still programs and maintains their boundaries. Joggle introduces \textbf{progressive IR}, in which implementation detail is exposed
...
blocks explicit; and \cite{lai2025relax} added symbolic
...
memories. Systems such as \cite{wang2024ladder},
\cite{ikarashi2022exo}, and
\cite{jain2026act} each make an important portion of this
...
search, while \cite{mai2026argus}
...
idea is \textbf{progressive IR}. A program does not migrate wholesale from graph to
...
that a progressive representation reduces \textbf{idea-to-artifact latency} when the
...
\begin{enumerate}
\item
\textbf{Progressive IR}, a representation in which abstract relations, reusable
...
\item
\textbf{A uniform compiler module mechanism} that lets independently installed
...
\item
\textbf{An idea-to-artifact evaluation methodology} that measures setup and change
...
\section{Motivation: shorten the path from idea to artifact}
\subsection{AI systems require continuous specialization}
...
This pattern is no longer exceptional. \cite{wu2025plus}
...
\subsection{New requirements have accumulated new IR boundaries}
...
\subsection{Generality and specialization should not be a choice}
...
The representation counterpart is \textbf{progressive specialization}. An abstract
...
\subsection{A new population of compiler authors}
...
structure. \cite{hong2025autocomp} uses domain knowledge,
correctness checks, and hardware measurements in every iteration of an
LLM-driven accelerator optimization loop. \cite{mai2026argus}
...
Joggle claims that progressive IR reduces \textbf{idea-to-artifact latency} for
...
\section{Progressive intermediate representation}
...
\subsection{RQ2: How quickly can an idea reach a checked artifact?}
