...
\acmYear{2027}
\title[Joggle: Typed Modules]{Joggle: Whole Compiler Extensions as Typed Modules}
...
extension systems and must be revised as a coordinated set. Joggle makes this complete object, a \textbf{whole compiler extension}, a typed,
...
\textbf{Progressive exposure} lets modules reveal implementation detail in place,
...
Joggle starts from a different observation: \textbf{cross-role decisions form one
...
\begin{enumerate}
\item
\textbf{Whole compiler extensions as typed modules:} vocabulary,
...
\item
\textbf{Progressive exposure}, a typed function representation in which abstract
...
\section{Motivation: make the extension whole}
...
\subsection{Existing systems modularize by role}
Existing systems cover the required capabilities, but package them around
different compiler roles. \cite{lattner2021mlir} makes dialects, interfaces, conversions, and passes reusable
across levels; \cite{jin2020onnxmlir} uses those mechanisms to connect model semantics to
native code and accelerator plugins. \cite{lai2025relax} lets graph functions, TensorIR
...
Automation supplies another producer of implementations. \cite{jain2026act} derives sound, complete tensor-
...
When those pieces are packaged separately, the resulting cost is \textbf{temporal
...
\subsection{Claim and measurable predictions}
Joggle makes whole extensions concrete through \textbf{progressive exposure}.
...
between many small, related languages. \cite{engler1997magik} dynamically loads application-specific
semantics and transformations with direct access to compiler IR. \cite{rompf2016lms} composes
...
\cite{tvmRuntimeDocs} are the strongest counterexample to a uniform-call
