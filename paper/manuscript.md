# Joggle: Progressive Exposure for Inference Compiler Experiments
...
leaving a partially transformed program.

We evaluate whether this uniformity survives real models and artifacts rather
...
Joggle tests a simple organizing rule: **keep the program stable; expose the
...
1. **Progressive function representation.** Source relations, reusable tensor
...
2. **A uniform extension boundary.** Source modules define semantic functions,
...
3. **An artifact-centered evaluation.** Frozen extension tasks and conventional
...
## 2. Motivation: a co-design experiment is a vertical change

### 2.1 The kernel is only one decision
...
surfaces. We call the end-to-end change a **vertical experiment**, and a point
where its intent must be manually re-encoded or reconstructed an **extension
discontinuity**. Our concern is not the existence of levels, but the accumulated
coupling a researcher must own across them.

### 2.2 Expert control and synthesis still need a common substrate
...
### 2.3 Edge inference turns abstraction loss into a performance cliff
...
### 2.4 Design requirements and falsifiable claim

The observations above imply four requirements. **Continuity:** imported calls,
...
**Replaceability:** a manually written rule and an automatic chooser must be
able to select the same decision without changing the host compiler.
**Explicit frontier:** unsupported semantics or capabilities must stop at a
named boundary rather than disappear into fallback code. **Artifact closure:**
...
seven dynamic positional-embedding calls (four `Expand`, three `Tile`). None
