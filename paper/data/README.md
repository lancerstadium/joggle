...
infers every result type and retains 396 ONNX calls after conversion. The
...
SSD-MobileNetV1 infers all result types, then retains 396 source-format calls
...
target-emitter case. A shared `nn.clip` composition removes another 35 Clip
calls through the same function-expansion path. The remaining calls include dynamic indexing, NonZero,
