...
Its original manifest is preserved byte-for-byte under
`paper/experiments/pilot/`, keeping the recorded digest verifiable after the
publication contract was strengthened to reject repeated system identities.

`mnist-systems-pilot.csv` and its JSON provenance record extend that runner to
a second independent model/system pair. At clean revision `fc12a26`, 20
balanced fresh-process trials compare generated C with ONNX Runtime 1.26.0 on
the same official MNIST model, input, reference output, and one-thread
contract. Median latencies are 0.910 ms and 0.050208 ms respectively, so
generated C is 18.12x slower. Both subjects pass their declared numerical
check. The run records a load-one admission threshold of 4, but it still lacks
thermal isolation and second-machine replication and therefore remains a
pilot.
Its original manifest is likewise preserved under `paper/experiments/pilot/`.
