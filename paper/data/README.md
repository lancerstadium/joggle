...
Median latencies are 167.315 ms and 10.346 ms respectively, so generated C is
16.17x slower. Both subjects pass their declared numerical check. The host was
...
The JSON record preserves the exact repository and manifest digests, host,
compiler, runtime, artifact hashes, thread environment, and raw-output digest.

`mnist-systems-pilot.csv` and its JSON provenance record extend that runner to
a second independent model/system pair. At clean revision `418a34e`, 20
balanced fresh-process trials compare generated C with ONNX Runtime 1.26.0 on
the same official MNIST model, input, reference output, and one-thread
contract. Median latencies are 0.530 ms and 0.050 ms respectively, so generated
C is 10.55x slower. Both subjects pass their declared numerical check. The run
still lacks thermal isolation and second-machine replication and therefore
remains a pilot.

`mobilenetv2-policy/` groups one clean Linux mechanism run at revision
`418a34e`. `variants.csv` records the common canonical IR, 54 reordered loop
bodies, every intermediate digest and stage duration, and the three emitted C
artifacts. Applying `tile.canon` after the generic reorder reduces source from
192,209 to 152,275 bytes, or 20.8%. `plain.csv` and `canon.csv` contain 20
balanced fresh-process trials against an adjacent ONNX Runtime subject; their
JSON companions preserve the complete provenance. Plain/canonical medians are
139.679/102.638 ms, a 26.5% reduction, while the canonical artifact remains
12.24x slower than the adjacent 8.382 ms ONNX Runtime median. This is a pinned
shared-runner mechanism result, not isolated publication performance.

`mobilenetv2-policy/replication/` preserves a separately dispatched run at
revision `b79c225`. It emits byte-identical model source, executable, weights,
input, and reference artifacts. Absolute plain/canonical medians shift to
178.796/131.557 ms, but the canonical/plain ratio changes only from 0.7348 to
...
are unchanged. MobileNetV2 medians are 173.787/7.345 ms for Joggle/ONNX
Runtime, a 23.66x gap; MNIST medians are 0.570/0.054 ms, a 10.59x gap. The
cross-CPU result reproduces correctness and the negative performance boundary,
...
rather than being mistaken for current capability. Publication tables must not
combine rows as one matched run unless their revision agrees. The ledger does
...
available only for pass regression and policy-boundary analysis.
