...
def main():
    ...
    fig = plt.figure(figsize=(3.33, 2.55))
    ...
    ax.text(0, 100, "(a) Representation", fontsize=11, color=INK, va="top")
    frame(ax, 0, 58, 100, 36, MOD, face="#F4F8FC", lw=1.0)
    ax.text(3, 90, "Mod", fontsize=11, color=MOD, va="center")
    ax.text(13, 90, "one program: definitions and data", fontsize=10,
            color=INK, va="center")
    frame(ax, 5, 62, 90, 24, MOD, lw=0.8)
    ax.text(8, 82, "Fn", fontsize=11, color=MOD, va="center")
    ax.text(16, 82, "typed, generic, overloadable", fontsize=10, color=INK,
            va="center")
    frame(ax, 10, 66, 80, 12, MOD, lw=0.8)
    ax.text(13, 74, "Blk", fontsize=10.5, color=MOD, va="center")
    ax.text(22, 74, "control flow", fontsize=9.6, color=INK, va="center")
    frame(ax, 15, 68, 70, 4, MOD, lw=0.8)
    ax.text(50, 70, "Op  →  Val", fontsize=9.6, color=INK, va="center")
    ...
    ax.text(0, 52, "(b) Roles across three modules", fontsize=11, color=INK, va="top")
    for x, tag, sub, color, face in ((0, "S", "installed  $f$", SUB, "#F4F6F8"),
        ...
        ax.text(x + 14, 33, tag, fontsize=12.5, color=INK, ha="center", va="center")
        ax.text(x + 14, 25, sub, fontsize=10, color=INK, ha="center", va="center")
    ax.add_patch(FancyArrowPatch((28.5, 30), (35.5, 30), arrowstyle="-|>",
                                 mutation_scale=8, color=DER, linewidth=1.1))
    ax.text(32, 44, "derive", fontsize=9.6, color=DER, ha="center", va="center")
    ax.add_patch(FancyArrowPatch((64.5, 30), (71.5, 30), arrowstyle="-|>",
                                 mutation_scale=8, color=MOD, linewidth=1.1))
    ax.text(68, 44, "invoke", fontsize=9.6, color=MOD, ha="center", va="center")

    ax.text(0, 14, "Both act on $P$, and neither replaces it.", fontsize=9.6,
            color=INK, va="center")
    ax.text(0, 8, "Edits to $K$ and to $P$ roll back separately.", fontsize=9.6,
            color=INK, va="center")
    ax.text(0, 2, "Package: module.jog + lib/*.jog (+ src/)", fontsize=9.6,
            color=INK, va="center")
