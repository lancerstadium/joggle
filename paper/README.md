...
| Extension surface | Four frozen contracts; all Joggle implementations pass; pinned TVM controls pass three contracts; ONNX-MLIR passes the implementation and policy tasks, while its documented external-call option has a preserved unsupported MatMul outcome | Specify and execute the numeric-format system boundary; repeat the ONNX-MLIR build without interruption from a clean checkout |
