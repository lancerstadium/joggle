  ...
  fn fuse_tensor(input: fn) -> fn {
    return @tr.pass(
      ...
      (
        make: (t.coord<[4]>) -> f32,
        map: (f32) -> f32
      ) -> t.tensor<f32, [4]> => t.map(t.build(make), map),
      (
        make: (t.coord<[4]>) -> f32,
        map: (f32) -> f32
  ...
  fn cancel_tensor(input: fn) -> fn {
    return @tr.pass(
      ...
      (
        make: (t.coord<[4]>) -> f32,
        position: t.coord<[4]>
      ) -> f32 => t.at(t.build(make), position),
      (
        make: (t.coord<[4]>) -> f32,
        position: t.coord<[4]>
      ) -> f32 => make(position)
...
int main() {
  ...
  ok &= expect(
      ...
      "the same typed pass fuses map(build(f), g) into one build with a "
  ...
  ok &= expect(cancelled_calls.size() == 1U &&
               ...
               "at(build(f), p) cancels through an ordinary typed equation "
